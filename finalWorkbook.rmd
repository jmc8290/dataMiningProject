...
For our classification algorithm, we have elected to do a random forest. To start, naturally we need to split our data into training and testing datasets
...
With that, we can now begin to fit our model
...
This section makes the predictions for the test data

```{r}
rf_pred_class <- predict(rf_model, newdata = test_data, type = "class")
```

Here is our confusion matrix. For whatever reason, the model decided that making less than 50k was the 'positive' class, however we calculated all of our metrics as making more than 50k being the positive class
...
Next we created our ROC curve using the standard library in R
...
One thing is nice is our ability to generate importance plots for the different features. Here we are grabbing plots for both mean decrease accuracy and Gini. However, we are limiting it to 5 and 10 variables as having all of the variables on the plot made it unreadable.
...
For our clustering algorithm, we are using a modified version of CLARA
...
Findings the best number of clusters based off a sample of 2000 from the main dataset 
...
2 clusters is essentially useless, so the next best appears to be 5. So that is what we chose. Next we divide the dataset in 3, take a 2000 size sample from each subset. We then run PAM on each of those samples. We then saved the labels found from PAM on the clusters
...
Next we do hungarian matching to make sure that we are using consistent labeling between the clusters. Since the clusters found basically the same groups, we just need to make sure the labels line up.
...
Now that the mappings are aligned, we assign each of the subsets clusters based off of their associated pam model.
...
Now we use our mappings to remap the clusters to make sure that every prediction is alligned 
...
Here we take the 3 subsets and combine them back into one full data set
...
To analyze the clusters, we'll filter the data to each of the clusters then analyze their characteristics. This essentially involved me scrolling through the data view and seeing where there was an abnormal number of positives in a column. After verifying by finding the proportion and comparing it to the respective proportion in the full dataset, we were able to assign identities that characterized the individuals in the cluster.
...
This was a piece of information that needed to have the scaling reversed. It is helpful as it helped make sense of some of the trends we see in the clusters
...
This section is essentially scratch work. It is where we made different plots and tables in order to have visualizations of the comparisons for the clusters
